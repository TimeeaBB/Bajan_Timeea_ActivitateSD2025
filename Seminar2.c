    ...
    *vectorNou = (struct Masina*)malloc((*dimensiune) * sizeof(struct Masina));
    ...
    for (int i = 0; i < nrElemente; i++)
    {
        if (vector[i].capacitateC > prag) {
            ...
            strcpy_s((*vectorNou)[index].marca,strlen(vector[i].marca)+1, vector[i].marca);
            index++;
        }
    }
}
...
int main() {
    ...
    vector[2] = initializare(3, 5, "Mazda", 50, '4');
    ...
    struct Masina masinaDacia = getPrimaMasinaDupaMarca(vector, nrElemente, "Mazda");
